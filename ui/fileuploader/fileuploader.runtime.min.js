TW.Runtime.Widgets.fileuploader=function(){function q(a){m=n=k=0;var f=c.getProperty("maximumFileSize"),d=c.getProperty("allowedFileTypes").toLowerCase();g=[];for(var b=0;b<a.length;b++){var e=0>=f||a[b].size/1048576<=f,p=0===d.length||-1!==d.indexOf("."+a[b].name.split(".").pop().toLowerCase());e&&p&&g.push(a[b])}if(g.length){c.jqElement.triggerHandler("UploadStarted");a={files:[]};f={files:[]};d=c.getProperty("path");for(b=0;b<g.length;b++)m+=g[b].size,e=d+(d.lastIndexOf("/")===d.length-1?"":"/")+
g[b].name,c.setProperty("fileName",g[b].name),c.setProperty("fullPath",e),a.files.push(g[b].name),f.files.push(e);c.setProperty("fileNames",JSON.stringify(a));c.setProperty("fullPaths",JSON.stringify(f));r()}else c.jqElement.triggerHandler("FilesNotCompatible")}function r(){var a=c.getProperty("debugMode"),f=new FormData;f.append("upload-repository-"+h,c.getProperty("repositoryName"));f.append("upload-path-"+h,c.getProperty("path"));f.append("upload-files-"+h,g[k]);var d=new XMLHttpRequest;d.open("POST",
"/Thingworx/FileRepositoryUploader",!0);d.setRequestHeader("X-XSRF-TOKEN","TWX-XSRF-TOKEN-VALUE");d.upload.addEventListener("progress",function(b){b=Math.floor(100*(n+b.loaded)/m);a&&console.log("FileUploader - progress - percentComplete = "+b);c.setProperty("progress",b);c.jqElement.triggerHandler("UploadProgress")},!1);d.onreadystatechange=function(){4===this.readyState&&(200===this.status?(n+=g[k].size,k++,k===g.length?(a&&console.log("FileUploader - success"),c.jqElement.triggerHandler("UploadComplete")):
